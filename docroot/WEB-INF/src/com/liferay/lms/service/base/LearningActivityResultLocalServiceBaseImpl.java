...
public abstract class LearningActivityResultLocalServiceBaseImpl
	implements LearningActivityResultLocalService, IdentifiableBean {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public LearningActivityResult addLearningActivityResult(
		...
		learningActivityResult = learningActivityResultPersistence.update(learningActivityResult,
				false);
		...
		if (indexer != null) {
			try {
				indexer.reindex(learningActivityResult);
			}
	...
	public void deleteLearningActivityResult(long larId)
		...
		LearningActivityResult learningActivityResult = learningActivityResultPersistence.remove(larId);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(learningActivityResult);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	...
	public void deleteLearningActivityResult(
		LearningActivityResult learningActivityResult)
		throws SystemException {
		learningActivityResultPersistence.remove(learningActivityResult);
	...
	private static Log _log = LogFactoryUtil.getLog(LearningActivityResultLocalServiceBaseImpl.class);
