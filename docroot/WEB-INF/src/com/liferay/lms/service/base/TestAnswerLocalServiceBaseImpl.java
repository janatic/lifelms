...
public abstract class TestAnswerLocalServiceBaseImpl
	implements TestAnswerLocalService, IdentifiableBean {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public TestAnswer addTestAnswer(TestAnswer testAnswer)
		...
		testAnswer = testAnswerPersistence.update(testAnswer, false);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.reindex(testAnswer);
			}
	...
	public void deleteTestAnswer(long answerId)
		...
		TestAnswer testAnswer = testAnswerPersistence.remove(answerId);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(testAnswer);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	...
	public void deleteTestAnswer(TestAnswer testAnswer)
		throws SystemException {
		testAnswerPersistence.remove(testAnswer);
	...
	public TestAnswer updateTestAnswer(TestAnswer testAnswer, boolean merge)
		...
		testAnswer = testAnswerPersistence.update(testAnswer, merge);
	...
	private static Log _log = LogFactoryUtil.getLog(TestAnswerLocalServiceBaseImpl.class);
